...
class _Host extends StatefulWidget {
  ...
  const _Host({required this.initialLabel});
            ...
            return ElevatedButton(
              ...
              onPressed: () async {
                final r = await showDialog<(String, String)>(
                  context: ctx,
                  builder: (_) => PinLabelDialog(initialLabel: widget.initialLabel),
                );
                setState(() => result = r);
              },
...
void main() {
  group('PinLabelDialog', () {
    ...
    testWidgets('Cancel button pops (cancel, "")', (tester) async {
      await open(tester, initial: 'hello');
      await tester.tap(find.text('Cancel'));
      await tester.pumpAndSettle();
      final state = tester.state<_HostState>(find.byType(_Host));
      expect(state.result, equals(('cancel', '')));
    });

    testWidgets('Delete button pops (delete, "")', (tester) async {
      await open(tester);
      await tester.tap(find.text('Delete'));
      await tester.pumpAndSettle();
      final state = tester.state<_HostState>(find.byType(_Host));
      expect(state.result, equals(('delete', '')));
    });

    testWidgets('OK button pops (ok, trimmed label)', (tester) async {
      await open(tester);
      await tester.enterText(find.byType(TextField), '  pole 7  ');
      await tester.tap(find.text('OK'));
      await tester.pumpAndSettle();
      final state = tester.state<_HostState>(find.byType(_Host));
      expect(state.result, equals(('ok', 'pole 7')));
    });
